...
<div id="kirim-pesan" class="bg-white mt-lg-0 ms-lg-4 mt-3 h-100 text-dark" style="padding-top: 64px;">
    <div class="container-fluid col-lg-12 px-4 py-md-5 py-2 ">
        ...
        <div class="border-bottom text-lg-start text-center mb-3">
            <h1 class="display-4 fw-bold lh-1 mb-3">Data PSKS</h1>
            <?= getFlash('message'); ?>
        </div>
        ...
        config = {
            ...
            buttons: {
                ...
                custom: {
                    ...
                    action: function() {
                        window.location.href = baseUrl('data/psks/tambah-spreadsheet');
                    }
                }
            },
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return (
                            `<button data-bs-id="${data}" data-bs-title="Foto ${row.community_name}" data-bs-toggle="modal" title="Lihat Gambar" data-bs-target="#image_modal" class="btn btn-sm btn-outline-secondary m-1 rounded">Foto</button><a href="${baseUrl(`data/psks/${data}`)}" class="btn m-1 btn-sm btn-primary rounded">Ubah</a>`
                        );
                    },
                },
            ],
