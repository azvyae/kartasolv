...
<nav id="main-navbar"
    class="navbar bg-gradient fw-semibold px-3 navbar-expand-lg <?=!isSameController(['Home']) ? 'bg-white shadow-sm' : '' ?> fixed-top">
    <div class="container-fluid col-11">
        <div class="me-5">
            <a href="<?= base_url(); ?>"
                class="navbar-brand  <?=!isSameController(['Home', 'Auth']) ? 'd-md-inline d-none' : '' ?>">
                <img src="<?= base_url('img/logo.webp'); ?>" alt="Logo Karang Taruna" width="48" height="48">
            </a>
            <?php if (!isSameController(['Home', 'Auth'])): ?>
                <button type="button" id="sidebarCollapse" class="navbar-toggler" data-bs-toggle="offcanvas"
        ...
        <div class="offcanvas offcanvas-end" style="width: 280px;" tabindex="-1" id="offcanvasNavbar"
            ...
            <div class="offcanvas-body  align-items-center justify-content-between">
                <ul class="navbar-nav text-center flex-grow-1 text-end pe-lg-3">
                    ...
                    <li class="nav-item py-2 py-md-0">
                        <a class="nav-link px-3 <?= isSamePage('sejarah') ?>"
                            href="<?= base_url('sejarah'); ?>">Sejarah</a>
                    </li>
                    <li class="nav-item py-2 py-md-0">
                        <a class="nav-link px-3 <?= isSamePage('hubungi-kami') ?>"
                            href="<?= base_url('hubungi-kami'); ?>">Hubungi Kami</a>
                    </li>
                    <?php if (checkAuth('userId')): ?>
                        <li class="nav-item py-2 py-md-0">
                            <a class="nav-link px-3 <?= isSamePage('dasbor') ?>"
                                href="<?= base_url('dasbor'); ?>">Dasbor</a>
                        </li>
                    <?php endif ?>
                    <li class="nav-item py-2 py-md-0">
                        <a class="nav-link px-3 <?= isSamePage('artikel') ?>"
                ...
                <ul class="navbar-nav pe-lg-3 text-lg-end text-center align-items-lg-center">
                    ...
                    <?php if (checkAuth('userId')): ?>
                        <li class="nav-item py-2 py-md-0 px-3">
                            ...
                            <span class="text-secondary"> Halo,
                                <?= getUserName(); ?>
                            </span>
                        </li>
                        <li class="nav-item dropdown-center py-2 py-md-0 px-3">
                            <a class="nav-link dropdown-toggle ms-3 ms-lg-0" href="#" role="button"
                                data-bs-toggle="dropdown" aria-expanded="false">
                                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor"
                                    class="bi text-secondary bi-person-circle" viewBox="0 0 16 16">
                            ...
                            <ul class="dropdown-menu text-center text-lg-start">
                                ...
                                <li>
                                    <form id="logoutAttempt" action="<?= base_url('keluar'); ?>" method="post">
                                        ...
                                        <button type="button" data-sitekey="<?= getCaptchaSitekey(); ?>"
                                            data-callback='onLogout' data-action='logout'
                                            class="dropdown-item g-recaptcha">Keluar</button>
                    ...
                    <?php else: ?>
                        ...
                        <?php if ($callToAction = getCalltoAction()): ?>
                            <a target="<?= $callToAction->target; ?>" href="<?= $callToAction->cta_url; ?>" id="callToAction"
                                class="btn px-3 my-1 btn-primary me-lg-2"><?= $callToAction->cta_text; ?></a>
                        <?php endif ?>
                    <?php endif ?>
