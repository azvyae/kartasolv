<!DOCTYPE html>
<html lang="id">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="<?= base_url('css/main.min.css'); ?>" rel="stylesheet">
    <link href="<?= base_url('css/style.css'); ?>" rel="stylesheet">
    <meta name="description" content="Sebagai organisasi sosial kepemudaan, Karang Taruna Ngajomantara Kelurahan Sarijadi merupakan wadah pembinaan dan pengembangan serta pemberdayaan dalam upaya mengembangkan kegiatan ekonomi produktif dengan pendayagunaan semua potensi yang tersedia di lingkungan baik sumber daya manusia maupun sumber daya alam yang telah ada untuk meningkatkan kesejahteraan sosial. Karang taruna melakukan kegiatan kegiatan pengembangan potensi dengan harapan tercapainya masyarakat yang yang baik dan tepat sasaran sesuai kebutuhan masyarakat.">
    <meta name="keywords" content="karang taruna, ngajomantara, kelurahan sarijadi, pelayanan sosial, <?= $title; ?>">
    <link rel="shortcut icon" type="image/x-icon" href="<?= base_url('favicon.ico'); ?>">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.9.1/font/bootstrap-icons.css">
    <title><?= $title; ?></title>
</head>

<body class="d-flex flex-column min-vh-100">
    <header style="margin-bottom: 74px;">
        <?= $this->include('layout/navbar'); ?>
    </header>
    <main class="my-auto">
        <?= $this->renderSection('main'); ?>
    </main>
    <footer class=" w-100 bg-light">

        <div class="container">
            <div class="py-3 my-4">
                <div class="d-flex pb-1 mb-1 justify-content-around align-items-center">
                    <a href="<?= base_url(); ?>" ">
                        <img src=" <?= base_url('img/logo.webp'); ?>" alt="Logo Karang Taruna" width="24" height="24">
                    </a>
                    <ul class="nav justify-content-center ">
                        <li class="nav-item <?php if (isSamePage('User\Home::index')) : ?>active<?php endif ?>"><a href="<?= base_url(); ?>" class="nav-link px-2 text-muted">Beranda</a></li>
                        <li class="nav-item <?php if (isSamePage('User\Home::index')) : ?>active<?php endif ?>"><a href="<?= base_url('sejarah'); ?>" class="nav-link px-2 text-muted">Sejarah</a></li>
                        <li class="nav-item <?php if (isSamePage('User\Home::index')) : ?>active<?php endif ?>"><a href="<?= base_url('hubungi-kami'); ?>" class="nav-link px-2 text-muted">Hubungi Kami</a></li>
                    </ul>
                    <a href="https://instagram.com/sarijadikarta" title="Instagram Kami" target="_blank">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi text-secondary bi-instagram" viewBox="0 0 16 16">
                            <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599.28.28.453.546.598.92.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598 2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233 0-2.136.008-2.388.046-3.231.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92.28-.28.546-.453.92-.598.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334z" />
                        </svg>
                    </a>
                </div>
                <hr>
                <p class="text-center text-muted">© <?= date("Y") ?> <span class="text-muted">Made With <i class="bi bi-heart-fill"></i> By <a target="_blank" href="https://linkedin.com/in/erstevn" class="text-decoration-none text-muted">Erstevn</a></span>
                </p>
            </div>
        </div>
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0/dist/js/bootstrap.bundle.min.js" integrity="sha384-A3rJD856KowSb7dwlZdYEkO39Gagi7vIsF0jrRAoQmDKKtQBHUuLZ9AsSv4jD4Xa" crossorigin="anonymous"></script>
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
    </footer>
</body>

</html>